...
- **Database Abstraction**: `DatabaseBackend` ABC in `src/db.py` wraps a single MySQL/MariaDB implementation (used by both Direct mode and, via `api/db.py`, the API server). `src/models.py` uses either API or Direct mode based on config.
